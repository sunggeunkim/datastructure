'''
reference - https://codefights.com/interview-practice/task/gxzW6jpXXkr3JNwXy
...
def productExceptSelf(nums, m):
    forward_prod = [1]*len(nums)
    backward_prod = 1
    for i in range(1,len(nums)):
        forward_prod[i] = (forward_prod[i-1] * nums[i-1]) % m
    s = 0
    for i in range(len(nums)):
        s = s + forward_prod[-i-1] * backward_prod        
        backward_prod = (backward_prod * nums[-i-1]) % m
    return s % m
